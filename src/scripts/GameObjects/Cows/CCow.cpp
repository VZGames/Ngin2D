...
CCow::CCow(float x, float y)
{
    ...
    m_box           = this->addComponent<engine::SBoxComponent>(E_BODY_TYPE::DYNAMIC, m_position->x, m_position->y, m_sprite->frameWidth, m_sprite->frameHeight);

    m_box->setVertex(Vector2DF(4, 28));
    m_box->setVertex(Vector2DF(28, 20));
